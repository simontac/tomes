...
async function fetchCollection () {
  // Retrieving the collection of books from the JSON file and parsing it
  ...
  // Display the number of books in collection inside page footer
  document.querySelector('.footer').innerHTML = `${collection.length} books in your collection`;

  // Sorting the collection array based on sorting-by-rating toggle state
  ...
  // Take the now sorted collection array and render the list via a series of div
  renderList(collection);
}

// Display the retrieved array of books from the JSON file as a list of divs
function renderList(retrievedBooksArray) {
  // Empty the list first to avoid repetiton of it
  const entriesList = document.querySelector(".entries-list");
  entriesList.innerHTML = "";
  
  // Create a div for each book in the collection array
  retrievedBooksArray.forEach(book => {
      ...
      bookEntry.innerHTML = `
          <div class="book-entry-main-info">
              ...
              <div class="book-rating">
                ${ratingConverter(book.rating)}
              </div>
          </div>
          <div class="book-entry-extra-info hidden">
              <div class="book-status">finished on ${book.dateRead}</div>
...
// Converting date from the JSON file to a usable date to sort books
